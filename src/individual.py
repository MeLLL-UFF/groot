...
class Individual:
    ...
    def define_splits(self, pos_target, neg_target, facts_target, fold_test_number):
        ...
        train_neg_target = []; test_pos_target = []
        train_facts_target = []; test_pos_target = []
        for index in range(0, len(pos_target)):
            if index == fold_test_number:
                test_pos_target = pos_target[index]
                test_neg_target = neg_target[index]
                test_facts_target = facts_target[index]
            else:
                train_pos_target.extend(pos_target[index])
                train_neg_target.extend(neg_target[index])
                train_facts_target.extend(facts_target[index])
