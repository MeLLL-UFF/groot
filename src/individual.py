...
class Individual:
	...
	def __init__(self, tree_source, bk_source, bk_target, pred_target):
	...
	def mapping_transfer(self, source_pred, target_pred):
		#source é do tipo: pred(A) e target é pred
		for pred in self.new_bk_source:
			if source_pred.split('(')[0] in pred and 'source: {}'.format(pred) not in self.transfer: 
		...
		for pred in self.new_bk_target:
			if target_pred in pred and 'target: {}'.format(pred) not in self.transfer:
				...
				break
		self.transfer.append('setMap: {}={}({}'.format(source_pred, target_pred, 
													   source_pred.split('(')[1]))
	...
	def get_bk_pred(self, bk, tree):
		pass

	def generate_map(self, individual_tree):
		tree_source = self.define_individuals(individual_tree)
	...
	def generate_random_individual(self, structured_tree):
		new_tree = []
		...
		for values, node in structured_tree[1].items():
			...
			self.mapping_transfer(node, valid_preds[index_choice])
	...
	def generate_random_individuals(self):
		...
		for index in range(0, len(self.tree_source)):
			...
			individual_structured_trees.append(self.generate_random_individual(structured_tree))
	...
	def construct_individual(self):
  	...
  	#gerar mapeamento entre predicados
  	#passar source original e o structured_trees modificados. pegar onde foi modificado e 
  	#fazer setMap.
  	#se forem iguais, modificar no refine para pred'
